...
st.header("where")
injure_people = st.slider("Number of persion in", 0, 19)
st.map(data.query("injured_persons>=@injure_people")
       [["latitude", "longitude"]].dropna(how="any"))

st.header("how many collusion occur")
hour = st.sidebar.slider("Hour to look at", 0, 23)
data = data[data['date/time'].dt.hour == hour]
st.markdown("Vehiclecollisions between %i:00 and %i:00" %
            (hour, (hour+1) % 24))
...
st.header("TOP 5 Dangerous steets by affected type ")
select = st.selectbox('affercted type of people', [
                      'Pedestrains', 'Cyclists', 'Motorists'])
